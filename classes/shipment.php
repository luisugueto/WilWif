...
class shipment{
	...
	function __construct($shipment_code_,$especial = false) 
	{
		...
		if($row = mysql_fetch_assoc($query))
		{
			...
			$this->shipment_user_to = $row["id_user_recive"];
			$sql = 'SELECT username FROM user where id ='.$this->shipment_user_from.'';
			$query = mysql_query($sql) or die('error at try to access data' . mysql_error());
			if($row2 = mysql_fetch_assoc($query))
			{
				$this->shipment_user_from = new userInfo($row2["username"]);
			}
			$sql = 'SELECT username FROM user where id ='.$this->shipment_user_to.'';
			$query = mysql_query($sql) or die('error at try to access data' . mysql_error());
			if($row2 = mysql_fetch_assoc($query))
			{
			...
			if($row2 = mysql_fetch_assoc($query))
			{
				$this->shipment_item = new item($row2["code"]);
			}

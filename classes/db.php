...
//$2y$10$uka1A.bW8FZEEYltklZCBeOgHF4CF1h55p0/LIqiFV88hmcOYbQlm
	...
	public function login($username, $password){
		$sql = 'SELECT username, password, id FROM user WHERE username = "'.$username.'" or email="'.$username.'"';
		...
		if($password == $row['password']){
		 	    ...
		 	    $_SESSION['id'] = $row['id'];
		 	    $history = "INSERT INTO history (id_user, action, date) VALUES('".$_SESSION['id']."', 'Login', NOW())";
	...
	public function loginBackOffice($username, $password){
		$sql = 'SELECT username, password, id, rol_id FROM user WHERE username = "'.$username.'" and status!="Block" and status!="Erased" and rol_id not in (select id from rol where code="001")';
		$query = mysql_query($sql) or die('error at try to access data' . mysql_error());
		$row = mysql_fetch_assoc($query);
